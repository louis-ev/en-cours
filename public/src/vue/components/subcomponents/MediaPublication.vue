<template>
  ...
  >
    <MediaContent
      ...
      :media="media"
      :element_width_for_sizes="mediaSize.width * 1"
      :read_only="read_only"
      :preview_size="800"
      v-model="media.content"
    />
    <transition name="slideup">
      <div class="m_media--caption" v-show="!!media.caption && is_hovered">{{ media.caption }}</div>
    </transition>
...
export default {
  ...
  methods: {
    ...
    updateMediaStyles() {
      this.mediaPos.x =
        this.media.hasOwnProperty("x") && !!Number.parseInt(this.media.x)
          ? this.limitMediaXPos(Number.parseInt(this.media.x))
          : this.page.margin_left;
      this.mediaPos.y =
        this.media.hasOwnProperty("y") && !!Number.parseInt(this.media.y)
          ? this.limitMediaYPos(Number.parseInt(this.media.y))
          : this.page.margin_top;
      this.rotate = this.media.hasOwnProperty("rotate") ? this.media.rotate : 0;
      this.mediaSize.width =
        this.media.hasOwnProperty("width") &&
        !!Number.parseInt(this.media.width)
          ? this.limitMediaWidth(Number.parseInt(this.media.width))
          : 100;
      this.mediaSize.height =
        this.media.hasOwnProperty("height") &&
        !!Number.parseInt(this.media.height)
          ? this.limitMediaHeight(Number.parseInt(this.media.height))
          : 100;
    },
